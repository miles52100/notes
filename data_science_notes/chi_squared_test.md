...
Suppose we have $n$ events that can occur as the outcome of one trial. We record the vector of counts of these events from $N$ independent trials as $(X_1,\ldots, X_n)$, so $X_i$ is the number of times we see event $i$ in $N$ trials. 

We have $\sum_{i=1}^{n}X_i=N$.
...
For a given significance level $\alpha$, we can set the rejection region using the following result.
...
We note that the marginal distribution of $X_1$ is $B(N,p_1)$. We can then use the normal approximation to the binomial to give us $X_1\sim N(Np_1,Np_1(1-p_1))$.
...
$$
and the result then follows, noting the degrees of freedom are $n-1$ rather than $n$ because $H_0$ has only $n-1$ linearly independent parameters.
...
Count the observations that fall into each of these intervals and call the counts $x_1,\ldots,x_n$. Then, under $H_0$, these counts should be a sample from a multinomial distribution with parameters $(1/n,\ldots,1/n)$ and so the appropriate test statistic is once again
